...
@if(error()){<div class="error-state"><app-icon name="info"/><h2>Chưa thể mở chương</h2><p>{{error()}}</p><button class="primary" (click)="load()">Thử lại</button><a routerLink="/">Về trang chủ</a></div>}
@if(data();as r){
<div class="reader-heading"><div class="breadcrumb"><a routerLink="/">Trang chủ</a><span>›</span><a [routerLink]="['/truyen-tranh',r.manga.id]">{{r.manga.title}}</a></div><h1>{{r.manga.title}} <span>— {{r.chapter.title}}</span></h1><p>{{r.chapter.group}} · {{pages().length}} trang · {{r.chapter.language==='vi'?'Tiếng Việt':'Tiếng Anh'}}</p></div>
<div class="reader-toolbar" [class.is-busy]="loading()"><a [routerLink]="['/truyen-tranh',r.manga.id]" class="icon-button" aria-label="Danh sách chương"><app-icon name="menu"/></a><button class="secondary" [disabled]="!previous()" (click)="move(-1)">‹ <span>Chương trước</span></button><select aria-label="Chọn chương" [ngModel]="r.chapter.id" (ngModelChange)="go($event)">@for(c of r.navigation;track c.id){<option [value]="c.id">{{c.title}}</option>}</select><button class="primary" [disabled]="!next()" (click)="move(1)"><span>Chương sau</span> ›</button><button class="icon-button" aria-label="Cài đặt trình đọc" (click)="showSettings=!showSettings"><app-icon name="settings"/></button></div>
...
<div class="reader-end"><h2>Bạn đã đọc hết {{r.chapter.title}}</h2><div><button class="secondary" [disabled]="!previous()" (click)="move(-1)">‹ Chương trước</button><button class="primary" [disabled]="!next()" (click)="move(1)">Chương sau ›</button></div><a [routerLink]="['/truyen-tranh',r.manga.id]">Về trang truyện và bình luận</a></div>
}`})
export class Reader {
 ...
 go(id:string){void this.router.navigate(['/chuong',id]);}
