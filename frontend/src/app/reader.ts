...
<div class="reader-pages" [style.max-width.px]="store.settings().width">@for(url of pages();track url;let i=$index){<div class="reader-page">@if(failed().has(i)){<div class="image-error"><p>Không tải được trang {{i+1}} từ máy chủ ảnh.</p><button class="secondary" (click)="retry(i)">Tải lại trang {{i+1}}</button></div>}@else{<img [src]="pageUrl(url, i)" [alt]="'Trang '+(i+1)+' — '+r.chapter.title" [loading]="i<2?'eager':'lazy'" referrerpolicy="no-referrer" (error)="imageError(i)">}</div>}</div>
...
export class Reader {
 api=inject(Api);store=inject(Store);route=inject(ActivatedRoute);router=inject(Router);data=signal<ReaderData|null>(null);loading=signal(true);error=signal('');failed=signal(new Set<number>());fallbackUrls=signal<Map<number,string>>(new Map());showSettings=false;id='';epoch=0;
 constructor(){this.route.paramMap.subscribe(p=>{this.id=p.get('id')!;void this.load();});}
 async load(){const n=++this.epoch;this.data.set(null);this.loading.set(true);this.error.set('');this.failed.set(new Set());this.fallbackUrls.set(new Map());try{const r=await this.api.request<ReaderData>('/chapters/'+this.id);if(n!==this.epoch)return;this.data.set(r);void this.store.record(r).catch(e=>this.store.notify(message(e)));window.scrollTo(0,0);}catch(e){if(n===this.epoch)this.error.set(message(e));}finally{if(n===this.epoch)this.loading.set(false);}}
 ...
 retry(i:number){this.fallbackUrls.update(m=>{const n=new Map(m);n.delete(i);return n;});this.failed.update(s=>{const n=new Set(s);n.delete(i);return n;});}
 quality(dataSaver:boolean){this.failed.set(new Set());this.fallbackUrls.set(new Map());this.store.saveSettings({...this.store.settings(),dataSaver});}
