...
<aside class="sidebar">
 <section><div class="section-title"><h2><app-icon name="clock"/> Lịch sử đọc truyện</h2><a routerLink="/lich-su">Xem tất cả</a></div>
 ...
 @for(h of store.history().slice(0,3);track h.mangaId){<a class="history-row" [routerLink]="['/chuong',h.chapterId]"><img [src]="h.cover" alt="" loading="lazy" decoding="async" referrerpolicy="no-referrer" (load)="$any($event.target).classList.add('loaded')" (error)="$any($event.target).classList.add('loaded')"><div><strong>{{h.title}}</strong><small>{{h.chapterTitle}}</small></div><app-icon name="chevron"/></a>}
 </section>
 ...
 @for(m of ranking();track m.id;let i=$index){<a class="rank-row" [routerLink]="['/truyen-tranh',m.id]"><span class="rank-number" [class.top]="i<3">{{i+1}}</span><img [src]="m.cover" [alt]="m.title" loading="lazy" decoding="async" referrerpolicy="no-referrer" (load)="$any($event.target).classList.add('loaded')" (error)="$any($event.target).classList.add('loaded')"><div><h3>{{m.title}}</h3><span><app-icon [name]="sort()==='rating'?'star':'heart'"/> {{sort()==='rating'?m.rating.toFixed(1):compact(m.follows)}}</span></div></a>}
 </section>
 <section><div class="section-title"><h2><app-icon name="comment"/> Bình luận gần đây</h2></div>@for(c of comments();track c.id){<div class="recent-comment"><div><span class="avatar">{{c.name.slice(0,1)}}</span><strong>{{c.name}}</strong><small>{{ago(c.createdAt)}}</small></div><p>{{c.body}}</p><a [routerLink]="['/truyen-tranh',c.mangaId]">{{c.mangaTitle}}</a></div>}@if(!comments().length){<p class="muted empty-small">Chưa có bình luận. Hãy bắt đầu cuộc trò chuyện!</p>}</section>
</aside>`})
export class Sidebar {
 ...
 async load(sort:string){const n=++this.epoch;this.sort.set(sort);this.loading.set(true);this.error.set('');try{const r=await this.api.request<Page<Manga>>('/catalog/search?'+this.api.query({sort,pageSize:7,language:this.store.settings().language}),'GET',undefined,true);if(n===this.epoch)this.ranking.set(r.items);}catch(e){if(n===this.epoch)this.error.set(message(e));}finally{if(n===this.epoch)this.loading.set(false);}}
}
